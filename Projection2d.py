...
def save_2d_proj(data, save_model_to, whichrun, e, i, type):
    ...
    ax1 = axarr[0, 0]
    ax1.axis('equal')
    plt.colorbar(ax1.imshow(capture_picture(dict(x=10, y=32, z=0, phi=3.875), data, 64, 64), cmap="hot"), ax=ax1)

    ax1 = axarr[0, 1]
    ax1.axis('equal')
    plt.colorbar(ax1.imshow(capture_picture({"x": 10, "y": 32, "z": 0, "phi": 5.425}, data, 64, 64), cmap="hot"), ax=ax1)
    ...
    ax1 = axarr[1, 0]
    ax1.axis('equal')
    plt.colorbar(ax1.imshow(capture_picture({"x": 32, "y": 40, "z": -10, "phi": 0}, data, 64, 64), cmap="hot"), ax=ax1)

    ax1 = axarr[1, 1]
    ax1.axis('equal')
    plt.colorbar(ax1.imshow(capture_picture({"x": 32, "y": 40, "z": -10, "phi": 3.1}, data, 64, 64), cmap="hot"), ax=ax1)
